...
from modules.telegram_bot import send_telegram_message, send_telegram_photo, save_config_to_env, get_current_config
...
with st.sidebar.expander("🖼️ Logo"):
    uploaded_logo = st.file_uploader("Unggah Logo Baru", type=["png", "jpg", "jpeg"])
...
with st.sidebar.expander("⚙️ Pengaturan API & Telegram", expanded=False):
    ...
    exchange = st.text_input("Nama Exchange", value=config["exchange"])
    api_key = st.text_input("API Key", value=config["api_key"])
    api_secret = st.text_input("API Secret", value=config["api_secret"], type="password")
    token = st.text_input("Telegram Bot Token", value=config["telegram_token"])
    chat_id = st.text_input("Telegram Chat ID", value=config["telegram_chat_id"])

    if st.button("📂 Simpan Pengaturan"):
        success = save_config_to_env(exchange, api_key, api_secret, token, chat_id)
        if success:
...
with st.sidebar.expander("⏱️ Pengaturan Sinyal", expanded=False):
    ...
    with st.sidebar.expander("🖼️ Pengaturan Screenshot", expanded=False):
        ...
        current_screenshot_interval = screenshot_interval_map[screenshot_interval_label]
...
def send_periodic_screenshot():
    ...
    while True:
        now = datetime.now()
        if (last_screenshot_time is None) or ((now - last_screenshot_time).total_seconds() >= screenshot_interval):
            try:
